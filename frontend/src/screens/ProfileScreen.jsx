...
const ProfileScreen = () => {
    ...
    return (
        <div className="transition-colors duration-300">
            ...
            <div
                className="relative pt-6 pb-24 px-6 rounded-b-[32px]"
                style={{
            ...
            <div className="mx-6 -mt-20 relative z-10 bg-white dark:bg-slate-800 rounded-3xl p-5 shadow-xl transition-colors duration-300">
                <div className="flex items-center gap-4 mb-4">
                    <div className="relative">
                        <div className="w-16 h-16 rounded-full bg-gradient-to-br from-gov-navy to-gov-safety-green flex items-center justify-center text-white text-xl font-bold border-4 border-white dark:border-slate-700 shadow-lg">
                            {formData.name?.charAt(0) || 'U'}
                        </div>
                        <button
                            onClick={() => setActiveModal('edit')}
                            className="absolute bottom-0 right-0 w-6 h-6 bg-gov-navy rounded-full flex items-center justify-center text-white shadow-lg hover:bg-gov-navy-dark transition-colors"
                        >
                            <Edit className="w-3 h-3" />
                        </button>
                    </div>
                    <div className="flex-1">
                        <h2 className="text-lg font-bold text-gray-900 dark:text-white leading-tight">{formData.name}</h2>
                        <p className="text-xs text-gray-500 dark:text-gray-400">{user?.id || 'ID: #WK-7829'}</p>
                        <p className="text-xs text-gov-navy dark:text-blue-400 font-semibold mt-0.5">{formData.role}</p>
                    </div>
                </div>

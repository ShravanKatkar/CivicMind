...
const DashboardPreview = () => {
    ...
    return (
        <section id="dashboard" className="py-16 md:py-24 bg-[#F7F9FC] dark:bg-slate-900/50 relative overflow-hidden">
            ...
            <div className="max-w-[1440px] mx-auto px-4 sm:px-6 md:px-12 w-full">
                ...
                <div className="text-center max-w-2xl mx-auto mb-10 md:mb-16">
                    ...
                    <h2 className="text-2xl sm:text-3xl md:text-4xl font-extrabold text-ink-navy dark:text-white font-display mb-4">
                ...
                {/* Dashboard Container */}
                <div className="bg-white dark:bg-slate-800 rounded-[20px] md:rounded-[28px] border border-gray-100 dark:border-slate-700/60 p-4 sm:p-6 md:p-8 shadow-soft relative">

                    {/* Header Controls */}
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-5 border-b border-gray-100 dark:border-slate-700/60 mb-5">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h3 className="font-extrabold text-ink-navy dark:text-white text-sm sm:text-base">CivicMind Live Safety Dashboard</h3>
                        ...
                        {/* Tabs */}
                        <div className="flex bg-[#F7F9FC] dark:bg-slate-900 p-1 rounded-xl border border-gray-200/50 dark:border-slate-800 self-start sm:self-auto">
                    ...
                    <div className="flex flex-col lg:grid lg:grid-cols-12 gap-5 lg:gap-8 lg:items-start">

                        {/* LEFT: Active Field Alerts */}
                        <div className="w-full lg:col-span-4 space-y-3">
                            ...
                            {/* On mobile, show as horizontal scroll chips; on lg show full cards */}
                            <div className="flex lg:hidden gap-2 overflow-x-auto pb-1 -mx-1 px-1 scrollbar-hide">
                                {incidents.map((incident, idx) => (
                                    <div
                                        key={idx}
                                        className={`flex-shrink-0 w-52 p-3 rounded-2xl border ${
                                    ...
                                    >
                                        ...
                                        <h5 className="font-bold text-xs text-ink-navy mb-0.5 leading-tight">{incident.site}</h5>
                                        <p className="text-[10px] font-semibold text-slate-gray">
                                            Worker: <span className="text-ink-navy font-bold">{incident.worker}</span>
                                        </p>
                                        <div className="flex items-center justify-between text-[10px] font-bold text-slate-gray pt-2 mt-2 border-t border-black/5">
                                            <span className="flex items-center gap-1"><MapPin className="w-3 h-3 text-electric-blue" /> Zone 2</span>
                                            <span className="text-alert-red font-data">{incident.risk}/10</span>
                                        </div>
                                    </div>
                                ))}
                            </div>
                        ...
                        <div className="w-full lg:col-span-5 flex flex-col gap-4">
                            ...
                            <div className="aspect-video bg-slate-900 rounded-2xl relative overflow-hidden border border-slate-800 shadow-inner flex items-center justify-center">
                                ...
                                <div className="absolute bottom-3 right-3 z-20 px-2.5 py-1 bg-black/60 backdrop-blur-md rounded-lg border border-white/10 text-[9px] font-mono text-emerald-400 font-bold uppercase tracking-wider">
                                    STATUS: NORMAL
                                </div>
                            </div>
                            ...
                            <div className="bg-sky-white rounded-2xl p-4 flex items-center justify-between border border-electric-blue/10">
                                <div className="flex items-center gap-3">
                                    <div className="w-9 h-9 flex-shrink-0 bg-electric-blue/15 rounded-xl flex items-center justify-center text-electric-blue">
                                ...
                                <span className="text-emerald-500 font-bold font-data text-sm flex-shrink-0">+2.4%</span>
                            </div>
                        </div>

                        {/* RIGHT: Telemetry Gauge — hidden on mobile, shown on lg */}
                        ...
                        {/* Mobile-only: Telemetry compact strip */}
                        <div className="lg:hidden w-full bg-white dark:bg-slate-800 rounded-2xl p-4 border border-gray-100 dark:border-slate-700/60 shadow-sm flex items-center gap-4">
                            <div className="w-12 h-12 flex-shrink-0 rounded-full bg-red-50 border-2 border-red-200 flex items-center justify-center">
                                <ShieldAlert className="w-5 h-5 text-alert-red" />
                            </div>
                            <div className="flex-1 min-w-0">
                                <span className="text-[10px] font-bold text-slate-gray uppercase tracking-wider block">Telemetry — Risk Score</span>
                                <h4 className="font-extrabold text-alert-red text-lg leading-none">8.2 <span className="text-xs font-bold text-slate-gray">/ 10</span></h4>
                                <p className="text-[10px] font-medium text-slate-gray mt-0.5 truncate">H2S gas approaching hazardous limit. PPE mandatory.</p>
                            </div>
                            <span className="flex-shrink-0 text-[9px] font-bold px-2 py-1 bg-red-100 text-alert-red rounded-full">CRITICAL</span>
                        </div>
